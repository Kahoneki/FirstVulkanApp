...
VkImageView ImageFactory::CreateImageView(const VkImage& _image, const VkFormat& _format)
{
	logger.Log(VK_LOGGER_CHANNEL::INFO, VK_LOGGER_LAYER::IMAGE_FACTORY,"Creating 1 Image View\n");
	return CreateImageViewImpl(_image, _format);
}



std::vector<VkImageView> ImageFactory::CreateImageViews(std::uint32_t _count, const VkImage* _images, const VkFormat* _formats)
{
	...
	for (std::size_t i{ 0 }; i<_count; ++i)
	{
		imageViews.push_back(CreateImageViewImpl(_images[i], _formats[i]));
	}
	return imageViews;
}
...
VkImage ImageFactory::AllocateImageImpl(const char* _filepath, const VkImageUsageFlags _flags, ImageMetadata* _out_metadata)
{
	...
	imgInfo.extent.width = static_cast<std::uint32_t>(imgData.metadata.width);
	imgInfo.extent.height = static_cast<std::uint32_t>(imgData.metadata.height);
	imgInfo.extent.depth = 1;
	imgInfo.mipLevels = 1;
	imgInfo.arrayLayers = 1;
	imgInfo.format = imgData.metadata.channels == 4 ? VK_FORMAT_R8G8B8A8_SRGB : (imgData.metadata.channels == 3 ? VK_FORMAT_R8G8B8_SRGB : (imgData.metadata.channels == 2 ? VK_FORMAT_R8G8_SRGB : VK_FORMAT_R8_SRGB));
	...
	barrier.srcAccessMask = 0; //No previous operation to wait on
	barrier.dstAccessMask = VK_ACCESS_TRANSFER_WRITE_BIT; //Wait on this barrier before allowing transfer write (copy) commands
	...
	//Transition layout from TRANSFER_DST_OPTIMAL to SHADER_READ_ONLY_OPTIMAL
	barrier.oldLayout = VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL;
	barrier.newLayout = VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL;
	barrier.srcAccessMask = VK_ACCESS_TRANSFER_WRITE_BIT; //Wait for the transfer write to finish
	barrier.dstAccessMask = VK_ACCESS_SHADER_READ_BIT; //The next operation will be a shader read
...
VkImageView ImageFactory::CreateImageViewImpl(const VkImage& _image, const VkFormat& _format)
{
	//Create image view
